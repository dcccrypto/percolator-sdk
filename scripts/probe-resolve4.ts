...
async function main() {
  ...
  // Build SetOracleAuthority + PushOraclePrice prefix ixs
  ...
  // Try various account combinations for resolve after price is set in-sim
  const base = { pubkey: admin.publicKey, isSigner: true, isWritable: true };
  const slabMeta = { pubkey: SLAB, isSigner: false, isWritable: true };

  // The deployed program may have a 3-account layout: admin, slab, oracle-price-account
  // where oracle-price-account is a Pyth/admin-price account
  // OR it may be: admin, slab, keeperFund, vault

  await tryResolve('Resolve 3-accts: admin+slab+vaultAuth (after set+push)', prefix, [
    base, slabMeta,
    { pubkey: vaultAuth, isSigner: false, isWritable: false },
  ]);

  await tryResolve('Resolve 3-accts: admin+slab+keeperFund', prefix, [
    base, slabMeta,
    { pubkey: keeperFund, isSigner: false, isWritable: true },
  ]);

  await tryResolve('Resolve 3-accts: admin+slab+vault', prefix, [
    base, slabMeta,
    { pubkey: VAULT, isSigner: false, isWritable: true },
  ]);

  await tryResolve('Resolve 4-accts: admin+slab+vault+vaultAuth', prefix, [
    base, slabMeta,
    { pubkey: VAULT, isSigner: false, isWritable: true },
    { pubkey: vaultAuth, isSigner: false, isWritable: false },
  ]);

  await tryResolve('Resolve 4-accts: admin+slab+adminAta+vault', prefix, [
    base, slabMeta,
    { pubkey: adminAta, isSigner: false, isWritable: true },
    { pubkey: VAULT, isSigner: false, isWritable: true },
  ]);

  await tryResolve('Resolve 6-accts: admin+slab+adminAta+vault+tokenProg+vaultAuth', prefix, [
    base, slabMeta,
    { pubkey: adminAta, isSigner: false, isWritable: true },
    { pubkey: VAULT, isSigner: false, isWritable: true },
  ...
  // Maybe ResolveMarket in the deployed V1 program has same layout as WithdrawInsurance
  // because the deployed binary combines resolve+withdraw (resolve closes all positions)
  // and then drains insurance automatically
  await tryResolve('Resolve 7-accts: admin+slab+adminAta+vault+tokenProg+vaultAuth+keeperFund', prefix, [
    base, slabMeta,
    { pubkey: adminAta, isSigner: false, isWritable: true },
    { pubkey: VAULT, isSigner: false, isWritable: true },
    { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
