...

{
  ...
  assert.throws(() => deriveLpPda(PROGRAM_ID, SLAB, 65_536), /lpIdx must be an integer/);
  assert.throws(() => deriveLpPda(PROGRAM_ID, SLAB, -1), /lpIdx must be an integer/);
  assert.throws(() => deriveLpPda(PROGRAM_ID, SLAB, 1.5), /lpIdx must be an integer/);
  assert.throws(() => deriveLpPda(PROGRAM_ID, SLAB, Number.NaN), /lpIdx must be an integer/);
  console.log("✓ deriveLpPda rejects out-of-range lpIdx");
}
...
{
  assert.throws(() => derivePythPushOraclePDA("abcd"), /64 hex digits/);
  assert.throws(() => derivePythPushOraclePDA(`${SAMPLE_FEED_64}00`), /64 hex digits/);
  assert.throws(() => derivePythPushOraclePDA(`${"a".repeat(63)}g`), /hexadecimal/);
